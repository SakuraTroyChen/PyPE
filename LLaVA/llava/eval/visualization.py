...
def get_token_attention(
    ...
    img_s = torch.where(input_ids_qu == -200)[1][0]
    # 注意这个是img的576个token还没插进去前的位置
    inst_end = torch.where(input_ids_qu == 13566)[1][0] + 1

    inst_len = inst_end - img_s

    attns = [attn.clone() for attn in out[0]["attentions"][0]]
    # [layer_id, bs, head_id, qk, v]
    attention_list = []

    for layer in tqdm(attns):
        # gen_to_sys = layer[0, :, img_s + 576 + inst_len :, :img_s]
        # gen_to_vis = layer[0, :, img_s + 576 + inst_len:, img_s : img_s + 576]
        # gen_to_inst = layer[0, :, img_s + 576 + inst_len:, img_s + 576 : img_s + 576 + inst_len ]
        # gen_to_gen = layer[0, :, img_s + 576 + inst_len:, img_s + 576 + inst_len:]

        # gen_to_sys = 5 * gen_to_sys.mean(dim=[0, 1, 2])
        # gen_to_vis = 5 * gen_to_vis.mean(dim=[0, 1, 2])
        # gen_to_inst = 5 * gen_to_inst.mean(dim=[0, 1, 2])
        # gen_to_gen = 5 * gen_to_gen.mean(dim=[0, 1, 2])

        # attention_list.append(
        #     {
        #         "gen_to_sys": gen_to_sys.tolist(),
        #         "gen_to_vis": gen_to_vis.tolist(),
        #         "gen_to_inst": gen_to_inst.tolist(),
        #         "gen_to_gen": gen_to_gen.tolist(),
        #     }
        # )

        inst_to_sys = layer[0, :, img_s + 576 : img_s + 576 + inst_len, :img_s]
        inst_to_vis = layer[
            0, :, img_s + 576 : img_s + 576 + inst_len, img_s : img_s + 576
        ]
        inst_to_inst = layer[
            0,
            :,
            img_s + 576 : img_s + 576 + inst_len,
            img_s + 576 : img_s + 576 + inst_len,
        ]

        inst_to_sys = 5 * inst_to_sys.mean(dim=[0, 1, 2])
        inst_to_vis = 5 * inst_to_vis.mean(dim=[0, 1, 2])
        inst_to_inst = 5 * inst_to_inst.mean(dim=[0, 1, 2])

        attention_list.append(
            {
                "gen_to_sys": inst_to_sys.tolist(),
                "gen_to_vis": inst_to_vis.tolist(),
                "gen_to_inst": inst_to_inst.tolist(),
            }
        )
...
def visualize_full_attention(
    ...
    tokens = (
        bos_tokens + p_before_tokens + ["img_token"] * NUM_IMAGE_TOKENS + p_after_tokens
    )
    ...
    for layer in range(len(attns)):
        ...
        attn_layer = attn_layer / attn_layer.sum(-1, keepdim=True)
        ...
        draw(5 * attn_layer.cpu().numpy(), x=tokens, y=tokens, ax=axs)
        plt.show()
        plt.savefig(
            f"{save_path}/full_{index}_layer{layer}.png",
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close()
...
def visualize_full_attention_in_group(
    model,
    tokenizer,
    input_ids,
    qu,
    ...
    for layer in range(len(attns)):
        attn_layer = attns[layer].max(1).values.data.squeeze()
        # aggreagated_attn_layer = attn_layer / attn_layer.sum(-1, keepdim=True)
...
def visualize_attention(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for layer_idx, layer in enumerate(output_scores):
            future = executor.submit(process_layer, layer_idx, layer)
...
def get_attention_score(
    ...
    for layer in attn_layers:
        ...
        # 平均了32个头的attention
        attn_list.append(score.mean(dim=[0, 1]).tolist())
