...
############# FOR_TRAIN_ONLY #############
# @register_llm("phi")
# def return_phiclass_for_train(tiny_config):
#     global IMG_SIZE, PATCH_SIZE, H, W, IMG_TOKEN_LEN, ONE_FLAG, CCA_FLAG, PRESET_IMG_TOKEN_LEN, JUMP_NUM, one_pos, concentric_pos

#     PhiForCausalLM.forward = phiforcausallm_forward
#     PhiForCausalLM.prepare_inputs_for_generation = prepare_inputs_for_generation

#     # pe_model_name_or_path = "pype4x"
...
#     if "pype" in pe_model_name_or_path:
#         transformers.models.phi.PhiModel.forward = pype_forward
#         model_type = pe_model_name_or_path.split("pype")[-1]
#         JUMP_NUM = int(model_type.split("x")[0])
#         print(
#             "################# Using PyPE with jump_num: {} #################".format(
#                 JUMP_NUM
#             )
#         )

#     elif "cca" in pe_model_name_or_path:
#         transformers.models.phi.PhiModel.forward = cca_forward
#         CCA_FLAG = True
...
#     else:
#         transformers.models.phi.PhiModel.forward = phimodel_forward
#         print("################# Using VANILLA #################")
...
@register_llm("phi")
def return_phiclass(tiny_config):
    global IMG_SIZE, PATCH_SIZE, H, W, IMG_TOKEN_LEN, ONE_FLAG, CCA_FLAG, PRESET_IMG_TOKEN_LEN, JUMP_NUM, one_pos, concentric_pos
        ...
        if batch_img_token_pos[0] == -1:
            ...
            for b_idx, img_token_pos in enumerate(batch_img_token_pos):
                ...
                if seq_len == 1:
                    ...
                    pype_position_ids = torch.ones(1).to(torch.long).to(self.device) * (
                    ...
                    pype_position_ids = (
                        torch.cat(
                            [
                                ...
                                pype_pos.flatten() + img_token_pos.to(pype_pos.device),
                                torch.arange(
                                    img_token_pos + preset_pype_img_token_len,
                                    seq_len - IMG_TOKEN_LEN + preset_pype_img_token_len,
                                ),
                            ]
                        )
                        .to(torch.long)
        ...
        if seq_len == 1:
            ...
            if batch_img_token_pos[0] == -1:
                ...
                if batch_pype_position_ids.shape[0] < batch_size:
                    ...
                    repeat_batch_pype_position_ids = batch_pype_position_ids.repeat(
