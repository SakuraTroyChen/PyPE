...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    def __init__(self, config: TinyLlavaConfig):
        ...
        self.language_model = LLMFactory(config)[0](config.text_config)
        ...
        (Tokenizer, post_load) = LLMFactory(config)[1]
    ...
    def forward(
        ...
        if self.training:
            if inputs_embeds is None:
                ...
                if (
                    len(batch_img_token_pos) == 0
                ):  # this happens when no image in input_ids.
                    batch_img_token_pos = (
                        torch.Tensor([-1] * input_ids.shape[0])
                        .to(batch_img_token_pos.device)
                        .to(batch_img_token_pos.dtype)
                    )
                (
                    input_ids,
                    position_ids,
