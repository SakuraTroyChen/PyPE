DATA_PATH=../playground/data/pretrain/blip_laion_cc_sbu_558k.json #pretrain annotation file path
FINETUNE_DATA_PATH=../playground/data/llava_v1_5_mix665k.json #finetune annotation file path
IMAGE_PATH=../playground/data/pretrain/images #pretrain image dir
FINETUNE_IMAGE_PATH=../playground/data #finetune image dir
...
bash scripts/train/pretrain.sh "$DATA_PATH" "$IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH" > logs/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}.out 2>&1
